...
#SBATCH -o /GWD/bioinfo/projects/RD-Cellzome_NGS/Analysis_Massimo_Petretich/tmp/PolyA-RNA-seq_out.txt
#SBATCH -e /GWD/bioinfo/projects/RD-Cellzome_NGS/Analysis_Massimo_Petretich/tmp/PolyA-RNA-seq_err.txt
...
mv $(basename "$fastqFile" | sed 's/\.txt\.gz/_fastqc\.html/') $outputDirectory/$outputFile'_fastqc_original.html'
...
mv $(echo $outputDirectory/$outputFile'_fastqc.html') $outputDirectory/$outputFile'_fastqc_trimmed.html'
##################################################################
...
R       --slave         --args          $outputDirectory/transcripts.gtf        $outputDirectory/$outputFile'_PolyA_transcripts.bed'  <       /GWD/bioinfo/projects/RD-Cellzome_NGS/Analysis_Massimo_Petretich/Software/Scripts/gtf_to_bed.R
...
for i in $( ls $outputDirectory | egrep -vi "^$outputFile.bam|summary.txt|$outputFile.*.bw|.html$|.pdf$|.RData$|deletions.bed|junctions.bed|transcripts.gtf|insertions.bed|transcripts.RData|transcripts.bed$" ); do if [ ! -d $outputDirectory/$i ]; then rm $outputDirectory/$i; fi; done
